...
import prmanToolsLighting_ui;reload(prmanToolsLighting_ui)
import prmanToolsShaderRename_ui;reload(prmanToolsShaderRename_ui)
import prmanToolsRibEdit_ui;reload(prmanToolsRibEdit_ui)
import prmanToolsTexureEdit_ui;reload(prmanToolsTexureEdit_ui)
import prmanToolsDelUnuse_ui;reload(prmanToolsDelUnuse_ui)
...
class prmanToolsRibEdit_ui(QtWidgets.QWidget,prmanToolsRibEdit_ui.Ui_main_widget):
    ...
    def ribFrameFix_PB_hit(self):
        ...
        rib_list=pm.ls(type='RenderManArchive')
        for i in rib_list:
            pm.connectAttr( 'global_timeUC.output', i+'.frame' ,f=True)
...
class prmanToolsDelUnuse_ui(QtWidgets.QWidget,prmanToolsDelUnuse_ui.Ui_main_widget):
    ...
    def loopDelNode(self,node_list):
        ...
        while len(self.checkTopAll(node_list,unused_node_list))>0:
            for node in node_list:
                if self.topNodeChcek(node,unused_node_list)==True:
    ...
    def topNodeChcek(self,node,unuseNode_list):
